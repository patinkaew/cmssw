...
void Run3ScoutingElectronArrayTableProducer::produce(edm::Event &iEvent, edm::EventSetup const &setup){
  ...
  for(auto &electron : *scouting_electron_handle){
    ...
    //assert((void(std::string("number of tracks mismatch: ")+std::to_string(n_trks)+std::to_string(" ")+std::to_string(electron.trkdz().size())+std::string(" ")+std::to_string(electron.trkpt().size()) + std::string(" ") + std::to_string(electron.trketa().size()) + std::string(" ") + std::to_string(electron.trkphi().size()) + std::string(" ") + std::to_string(electron.trkchi2overndf().size()) + std::string(" ") + std::to_string(electron.trkcharge().size())), 
    assert((void(TString::Format("number of tracks mismatch: %ld %ld %ld %ld %ld %ld %ld", n_trks, electron.trkdz().size(), electron.trkpt().size(), electron.trketa().size(), electron.trkphi().size(), electron.trkchi2overndf().size(), electron.trkcharge().size()).Data()),
  ...
  track_table->addColumn<float>("trkdz", trkdz, "flatten list of electron's associated track dz");
  track_table->addColumn<float>("trkpt", trkpt, "flatten list of electron's associated track pt");
  track_table->addColumn<float>("trketa", trketa, "flatten list of electron's associated track eta");
  track_table->addColumn<float>("trkphi", trkphi, "flatten list of electron's associated track phi");
  track_table->addColumn<float>("trkpMode", trkpMode, "flatten list of electron's associated track pMode");
  track_table->addColumn<float>("trketaMode", trketaMode, "flatten list of electron's associated track etaMode");
  track_table->addColumn<float>("trkphiMode", trkphiMode, "flatten list of electron's associated track phiMode");
  track_table->addColumn<float>("trkqoverpModeError", trkqoverpModeError, "flatten list of electron's associated track qoverpModeError");
  track_table->addColumn<float>("trkchi2overndf", trkchi2overndf, "flatten list of electron's associated track chi2overndf");
  track_table->addColumn<int>("trkcharge", trkcharge, "flatten list of electron's associated track charge");
