...
#include "PhysicsTools/NanoAOD/interface/Cut.h"
...
public:
  ...
  SimpleOrbitFlatTableProducer(edm::ParameterSet const &params)
      : name_(params.getParameter<std::string>("name")),
        ...
        cuts_(params.existsAs<edm::ParameterSet>("cuts")
                ? params.getParameter<edm::ParameterSet>("cuts")
                : edm::ParameterSet(),
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    desc.addOptional<edm::ParameterSetDescription>("cuts", Cuts<T>::cutsDescription());
      ...
      if (singleton_) {
        ...
        for (const unsigned int &bx : src->getFilledBxs()) {
          ...
          for (unsigned int i = 0; i < objs.size(); i++) {
            ...
            if (cut_(obj) && cuts_(objptr)) { // apply cut
