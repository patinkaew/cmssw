    ...
    namespace mahi {
      ...
      public:
        ...
        ALPAKA_FN_ACC void operator()(TAcc const& acc,
                                      ...
                                      float* soiSamples) const {
          auto const startingSample = f01HEDigis.stride() - 1;
          ...
          for (auto group : uniform_groups_y(acc, nchannels)) {
            ...
            for (auto channel : uniform_group_elements_y(acc, group, nchannels)) {
              ...
              for (auto i_sample : independent_group_elements_x(acc, nsamplesForCompute)) {
                ...
                //auto const nsamples = gch < f01HEDigis.size() ? compute_nsamples<Flavor1>(stride)
                ...
                auto const adc = gch < f01HEDigis.size()
                                     ? adc_for_sample<Flavor1>(f01HEDigis.data()->data() + stride * gch, sample)
                                     : (gch < nchannelsf015
                                            ? adc_for_sample<Flavor5>(
                                                  f5HBDigis.data()->data() + stride * (gch - f01HEDigis.size()), sample)
                                            : adc_for_sample<Flavor3>(
                                                  f3HBDigis.data()->data() + stride * (gch - nchannelsf015), sample));
                auto const capid =
                    gch < f01HEDigis.size()
                        ? capid_for_sample<Flavor1>(f01HEDigis.data()->data() + stride * gch, sample)
                        : (gch < nchannelsf015
                               ? capid_for_sample<Flavor5>(
                                     f5HBDigis.data()->data() + stride * (gch - f01HEDigis.size()), sample)
                               : capid_for_sample<Flavor3>(f3HBDigis.data()->data() + stride * (gch - nchannelsf015),
                                                           sample));
                ...
                if (gch < f01HEDigis.size()) {
                  ...
                  auto const soibit = soibit_for_sample<Flavor1>(f01HEDigis.data()->data() + stride * gch, sample);
                  if (soibit == 1)
                    soiSamples[gch] = sampleWithinWindow;
                } else if (gch >= nchannelsf015) {
                  auto const soibit =
                      soibit_for_sample<Flavor3>(f3HBDigis.data()->data() + stride * (gch - nchannelsf015), sample);
                  if (soibit == 1)
                    soiSamples[gch] = sampleWithinWindow;
                }
                ...
                if (gch >= f01HEDigis.size() && gch < nchannelsf015) {
                  ...
                  if (gch < f01HEDigis.size())
                    tdcTime = HcalSpecialTimes::getTDCTime(
                        tdc_for_sample<Flavor1>(f01HEDigis.data()->data() + stride * gch, sample));
                  else if (gch >= nchannelsf015)
                    tdcTime = HcalSpecialTimes::getTDCTime(
                        tdc_for_sample<Flavor3>(f3HBDigis.data()->data() + stride * (gch - nchannelsf015), sample));
                }
                ...
                if (sampleWithinWindow == static_cast<unsigned>(soi)) {
                  ...
                  float const max_energy = float(val & 0xffffffff);
              ...
              for (auto ipulse : independent_group_elements_y(acc, npulses)) {
                ...
                for (auto sample : independent_group_elements_x(acc, nsamples)) {
                  ...
                  auto const id = gch < f01HEDigis.size()
                                      ? f01HEDigis.ids()[gch]
                                      : (gch < nchannelsf015 ? f5HBDigis.ids()[gch - f01HEDigis.size()]
                                                             : f5HBDigis.ids()[gch - nchannelsf015]);
                  //auto const id = gch >= f01HEDigis.size()
                  //    ? idsf5HB[gch - f01HEDigis.size()]
                  //    : idsf01HE[gch];
  ...
  namespace hcal {
    namespace reconstruction {
      ...
      void entryPoint(Queue& queue,
        ...
        //WORDS_PER_SAMPLE = 1
        //auto const f01HEDigisStride = f01HEDigis->metadata().size() * WORDS_PER_SAMPLE + HEADER_WORDS;
        //auto const f5HBDigisStride = f5HBDigis->metadata().size() * WORDS_PER_SAMPLE + HEADER_WORDS;
        //auto const f3HBDigisStride = f3HBDigis->metadata().size() * WORDS_PER_SAMPLE + HEADER_WORDS;
        ...
        std::cout << "threads: " << threads_3d.x << " " << threads_3d.y << "  " << threads_3d.z << std::endl;
        std::cout << "blocks: " << blocks_3d << std::endl;
#endif
