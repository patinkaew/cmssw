              ...
              for (auto i_sample : independent_group_elements_x(acc, nsamplesForCompute)) {
                ...
                if (sampleWithinWindow == static_cast<unsigned>(soi)) {
                  ...
                  if (gch >= f01HEDigis.size() && gch < nchannelsf015) {
                        ...
                        if (gch < f01HEDigis.size())
                          tdcTime = HcalSpecialTimes::getTDCTime(tdc_for_sample<Flavor1>(&(f01HEDigis.data()[gch][0]), sample),subdetectorType, subdetectorDepth);
                        else if (gch >= nchannelsf015)
                          tdcTime = HcalSpecialTimes::getTDCTime(tdc_for_sample<Flavor3>(&(f3HBDigis.data()[gch - nchannelsf015][0]), sample),subdetectorType, subdetectorDepth);
                  }
